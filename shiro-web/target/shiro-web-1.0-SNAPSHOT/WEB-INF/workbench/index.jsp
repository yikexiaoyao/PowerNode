		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary" data-dismiss="modal"
						onclick="window.location.href='${pageContext.request.contextPath}/login.html';">更新
				</button>
			</div>
		</div>
	</div>
	...
	<div class="modal-dialog" role="document" style="width: 30%;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
				<button type="button" class="btn btn-primary" data-dismiss="modal"
						onclick="window.location.href='${pageContext.request.contextPath}/user/logOut ';">确定
				</button>
			</div>
		</div>
	</div>
...
<div id="top" style="height: 50px; background-color: #3C3C3C; width: 100%;">
	<div style="position: absolute; top: 5px; left: 0px; font-size: 30px; font-weight: 400; color: white; font-family: 'times new roman'">
		shiro<span style="font-size: 12px;">&copy;2021&nbsp;王者峡谷</span></div>
	...
	<div id="navigation" style="left: 0px; width: 18%; position: relative; height: 100%; overflow:auto;">
		...
		<ul id="no1" class="nav nav-pills nav-stacked">
			...
			<c:forEach items="${menus}" var="menu">
				<c:choose>
					<c:when test="${empty menu.sons}">
						<li class="liClass"><a href="${menu.url}" target="workareaFrame"><span
								class="glyphicon glyphicon-home"></span> ${menu.pname}</a></li>
					</c:when>
					<c:otherwise>
						<li class="liClass">
							<a href="#no2" class="collapsed" data-toggle="collapse"><span
									class="glyphicon glyphicon-stats"></span> ${menu.pname}</a>
							<ul id="no2" class="nav nav-pills nav-stacked collapse">
								<c:forEach items="${menu.sons}" var="sonMenu">
									<li class="liClass"><a href="${sonMenu.url}" target="workareaFrame">&nbsp;&nbsp;&nbsp;<span
											class="glyphicon glyphicon-chevron-right"></span> ${sonMenu.pname}</a></li>
								</c:forEach>
							</ul>
						</li>
					</c:otherwise>
