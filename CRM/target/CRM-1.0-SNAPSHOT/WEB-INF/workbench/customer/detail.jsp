...
<body>
	...
	<script>
		...
		function refresh(customerRemark){
			$('#remarkDiv').before("<div class=\"remarkDiv\" id="+customerRemark.id+" style=\"height: 60px;\">\n" +
					...
					"\t\t\t\t<font color=\"gray\">客户</font> <font color=\"gray\">-</font> <b>"+customerRemark.customerId+"</b> <small style=\"color: gray;\"> "+customerRemark.createTime+" 由"+customerRemark.createBy+"</small>\n" +
					"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
					"\t\t\t\t\t<a class=\"myHref\" onclick=\"openEditRemarkModal('"+customerRemark.id+"','"+customerRemark.noteContent+"')\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
					"\t\t\t\t\t&nbsp;&nbsp;&nbsp;&nbsp;\n" +
		...
		//异步查询详情页数据
		$.get('${pageContext.request.contextPath}/workbench/customer/toDetail',{'id':'${id}'},function (data) {
			//data:Customer
			//先设置客户数据(只设置两项)<b></b> text() html()
			var customer = data;
			...
			$('#describe').text(customer.description);
			$('#address').text(customer.address);

			//取出客户的备注数据
			var customerRemarks = customer.customerRemarks;
			for(var i in customerRemarks){
		...
		function saveRemark() {
			...
			$.post("${pageContext.request.contextPath}/workbench/customer/saveRemark",{
		...
		function deleteRemark(id) {
			$.get("${pageContext.request.contextPath}/workbench/customer/deleteRemark",{'id':id},function (data) {
				//data:resultVo
				if(data.ok){
					alert(data.message);
		...
		$('#updateRemarkBtn').click(function () {
			$.get("${pageContext.request.contextPath}/workbench/customer/updateRemark",{
				'id':$('#id').val(),//备注的主键
