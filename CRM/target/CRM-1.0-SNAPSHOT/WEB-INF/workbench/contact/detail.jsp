...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
							<label for="edit-contactName" class="col-sm-2 control-label">客户名称</label>
							<div class="col-sm-10" style="width: 300px;">
								<input type="text" class="form-control" id="edit-contactName" placeholder="支持自动补全，输入客户不存在则新建" value="动力节点">
							</div>
						</div>
	...
	<script>
		...
		function refresh(contactRemark){
			$('#remarkDiv').before("<div class=\"remarkDiv\" id="+contactRemark.id+" style=\"height: 60px;\">\n" +
					"\t\t\t<img title=\"zhangsan\" src="+contactRemark.img+" style=\"width: 30px; height:30px;\">\n" +
					"\t\t\t<div style=\"position: relative; top: -40px; left: 40px;\" >\n" +
					"\t\t\t\t<h5 id=h5"+contactRemark.id+">"+contactRemark.noteContent+"</h5>\n" +
					"\t\t\t\t<font color=\"gray\">联系人</font> <font color=\"gray\">-</font> <b>"+contactRemark.contactId+"</b> <small style=\"color: gray;\"> "+contactRemark.createTime+" 由"+contactRemark.createBy+"</small>\n" +
					"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
					"\t\t\t\t\t<a class=\"myHref\" onclick=\"openEditRemarkModal('"+contactRemark.id+"','"+contactRemark.noteContent+"')\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
					"\t\t\t\t\t&nbsp;&nbsp;&nbsp;&nbsp;\n" +
					"\t\t\t\t\t<a class=\"myHref\" onclick=\"deleteRemark('"+contactRemark.id+"')\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-remove\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
					"\t\t\t\t</div>\n" +
					"\t\t\t</div>\n" +
					"\t\t</div>");
		...
		//异步查询详情页数据
		$.get('${pageContext.request.contextPath}/workbench/contact/toDetail',{'id':'${id}'},function (data) {
			//data:Contact
			//先设置联系人数据(只设置两项)<b></b> text() html()
			var contact = data;
			$('#owner').text(contact.owner);
			$('#source').text(contact.source);
			$('#customerName').text(contact.customerName);
			...
			$('#describe').text(contact.description);
			...
			for(var i in contactRemarks){
				var contactRemark = contactRemarks[i];
		...
		function saveRemark() {
			...
			$.post("${pageContext.request.contextPath}/workbench/contact/saveRemark",{
		...
		function deleteRemark(id) {
			$.get("${pageContext.request.contextPath}/workbench/contact/deleteRemark",{'id':id},function (data) {
				//data:resultVo
				if(data.ok){
					alert(data.message);
		...
		$('#updateRemarkBtn').click(function () {
			$.get("${pageContext.request.contextPath}/workbench/contact/updateRemark",{
				'id':$('#id').val(),//备注的主键
				'noteContent' : $('#noteContent').val()//用户修改的备注的内容
			},function (data) {
				//data:resultVo
				if(data.ok){
					alert(data.message);
