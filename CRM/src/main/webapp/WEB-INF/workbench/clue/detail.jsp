...
<body>
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<div class="form-group has-feedback">
							<input type="text" class="form-control" style="width: 300px;" id="name" placeholder="请输入市场活动名称，支持模糊查询">
	...
	<div style="position: relative; top: -70px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 70px;">
			...
			<div style="width: 630px;position: relative; left: 200px; top: -20px;"><b id="decribe"></b></div>
	...
	<div style="position: relative; top: 40px; left: 40px;">
		...
		<%--<div class="remarkDiv" style="height: 60px;">
			<img title="zhangsan" src="${pageContext.request.contextPath}/img/user-thumbnail.png" style="width: 30px; height:30px;">
			<div style="position: relative; top: -40px; left: 40px;" >
				<h5>哎呦！</h5>
	...
	<script>
		//刷新备注
		function refresh1(clueRemark){
			$('#remarkDiv').before("<div class=\"remarkDiv\" id="+clueRemark.id+" style=\"height: 60px;\">\n" +
					...
					"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
					"\t\t\t\t\t<a class=\"myHref\" onclick=\"openEditRemarkModal('"+clueRemark.id+"','"+clueRemark.noteContent+"')\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
					"\t\t\t\t\t&nbsp;&nbsp;&nbsp;&nbsp;\n" +
					"\t\t\t\t\t<a class=\"myHref\" onclick=\"deleteRemark('"+clueRemark.id+"')\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-remove\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
					"\t\t\t\t</div>\n" +
					"\t\t\t</div>\n" +
					"\t\t</div>");
		...
		//异步查询线索详情页数据
		$.get('${pageContext.request.contextPath}/workbench/clue/toDetail',{'id':'${id}'},function (data) {
			//data:Clue
			// 先设置线索数据
			var clue = data;
			$('#fullname').text(clue.fullname);
			...
			$('#createBy').text(clue.createBy);
			$('#createTime').text(clue.createTime);
			$('#editBy').text(clue.editBy);
		...
		function saveRemark() {
			...
			$.post("${pageContext.request.contextPath}/workbench/clue/saveRemark", {
		...
		function deleteRemark(id) {
			$.get("${pageContext.request.contextPath}/workbench/clue/deleteRemark",{'id': id} ,function (data) {
				//data:resultVo
				if (data.ok) {
					alert(data.message);
		...
		//刷新关联的市场活动数据
		function refresh2(activity){
			$('#relationBody').append("<tr id="+activity.id+">\n" +
					"\t\t\t\t\t\t\t<td>"+activity.name+"</td>\n" +
		...
		$('#name').keypress(function (event) {
			if(event.keyCode == 13){
				$.get('${pageContext.request.contextPath}/workbench/clue/searchActivity',{
					'name' : $(this).val(),
					'id' : '${id}',
				},function (data) {
					//data:List<Activity>
		...
		function bind() {
			...
			$.get('${pageContext.request.contextPath}/workbench/clue/bind',{
				'id' : '${id}',//线索id
				'ids' : ids.join()
			},function (data) {
				//data:resultVo
